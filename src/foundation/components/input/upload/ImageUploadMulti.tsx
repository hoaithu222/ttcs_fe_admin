    ...
    <div className={`space-y-3 ${className}`}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        ...
        {images.map((image, index) => (
          ...
          >
            <div className="w-full h-full overflow-hidden rounded-xl border-2 border-border-2 shadow-sm transition-all duration-200 hover:shadow-md">
              <img
                src={image.url}
                alt={`Upload ${index + 1}`}
              ...
              {!disabled && (
                <div className="absolute inset-0 bg-overlay opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-error hover:bg-error/90 text-neutral-0 text-xs font-medium rounded-lg shadow-lg transition-all duration-200 hover:scale-105"
                  >
          ...
          <div
            className={`relative aspect-square border-2 border-dashed rounded-xl transition-all duration-200 ${
              disabled
                ...
                : "border-input-border hover:border-primary-4 hover:bg-primary-1/50 cursor-pointer active:scale-[0.98]"
            } ${isUploading && "border-primary-4 bg-primary-1/50"} ${
              (error || uploadError) && "border-error-3 bg-error-1/30"
            }`}
            onClick={handleClick}
          >
            <input
            ...
            {isUploading ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 p-4">
                <div className="w-8 h-8 border-3 border-primary-5 border-t-transparent rounded-full animate-spin" />
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 p-4 text-center">
                <div className="p-2.5 bg-gradient-to-br from-primary-5 to-primary-6 rounded-full shadow-lg">
                  <Upload className="w-5 h-5 text-neutral-0" />
                </div>
                <div>
                  <p className="text-xs font-semibold text-neutral-7">Add Images</p>
      ...
      {/* Error Message */}
      {(error || uploadError) && (
        <div className="flex items-start gap-2 p-3 bg-error-1 rounded-lg border border-error-2">
          <svg
          ...
          <p className="text-xs font-medium text-error-7">{error || uploadError}</p>
        </div>
      )}
