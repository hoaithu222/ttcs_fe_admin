...
const ImageUpload: React.FC<ImageUploadProps> = ({
  ...
  return (
    <div className={clsx("space-y-2", className)} data-testid={testId}>
      ...
      >
        {value ? (
          <div className="relative group">
            <div
              className={clsx(
                "overflow-hidden relative rounded-xl border-2 border-border-2 shadow-sm transition-all duration-200 hover:shadow-md",
              ...
              {!disabled && (
                <div className="flex absolute inset-0 justify-center items-center opacity-0 transition-opacity duration-200 bg-overlay group-hover:opacity-100">
                  <button
                    ...
                    className="flex gap-2 items-center px-4 py-2 text-sm font-medium text-neutral-0 bg-error rounded-lg shadow-lg transition-all duration-200 hover:bg-error/90 hover:scale-105"
            ...
            {isUploading ? (
              ...
              >
                <div className="p-3 bg-gradient-to-br from-primary-5 to-primary-6 rounded-full shadow-lg">
                  <ImageUp className="w-7 h-7 text-neutral-0" />
                </div>
                <div>
                  <p className="text-sm font-semibold text-neutral-7">Click to upload</p>
                  <p className="mt-1 text-xs text-neutral-5">PNG, JPG, GIF up to {maxSizeInMB}MB</p>
                </div>
              </div>
            )}
          </div>
      ...
      {(error || uploadError) && (
        <div className="flex gap-2 items-start p-3 bg-error-1 rounded-lg border border-error-2">
          <svg
