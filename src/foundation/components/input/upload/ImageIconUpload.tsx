...
const ImageIconUpload: React.FC<ImageIconUploadProps> = ({
  ...
  return (
    <div className={clsx("inline-flex flex-col gap-2", className)} data-testid={testId}>
      ...
      <div className="relative inline-block">
        <div
          className={clsx(
            ...
            isUploading && "border-primary-4 bg-primary-1",
            displayError && "border-error-3 bg-error-1",
            !value &&
              !isUploading &&
              !displayError &&
              "border-dashed border-input-border hover:border-primary-4 hover:bg-primary-1/30"
          )}
          onClick={handleClick}
            ...
            <div className="relative w-full h-full group">
              ...
              {isHovering && !disabled && (
                <div className="flex absolute inset-0 justify-center items-center bg-overlay backdrop-blur-[1px]">
                  <ImageIcon className={clsx("text-neutral-0", config.uploadIcon)} strokeWidth={2} />
                </div>
              )}
            </div>
          ) : (
            ...
            <div className="flex absolute inset-0 flex-col gap-1 justify-center items-center">
              <div
                className={clsx(
                  ...
                  isHovering ? "bg-primary-5 scale-110" : "bg-neutral-2"
                )}
              >
                <ImageUp
                  className={clsx(config.uploadIcon, isHovering ? "text-neutral-0" : "text-neutral-5")}
                  strokeWidth={2}
                />
              </div>
              <span className={clsx(config.text, "font-medium text-neutral-4 text-center px-1")}>
                {placeholder}
              </span>
            </div>
          )}
        ...
        {value && !disabled && showRemoveButton && (
          ...
          >
            <X className={clsx("text-neutral-0", config.removeIcon)} strokeWidth={2.5} />
          </button>
        )}
      </div>
      ...
      {/* Error Message */}
      {displayError && (
        <div className="flex gap-1.5 items-start p-2 max-w-xs bg-error-1 rounded-lg border border-error-2">
          <svg
          ...
          <p className="text-xs font-medium text-error-7 leading-tight">{displayError}</p>
        </div>
      )}
