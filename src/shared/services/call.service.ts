...
export class CallService {
  ...
  async initiateCall(conversationId: string, callType: "voice" | "video"): Promise<void> {
    ...
    try {
      ...
      // Note: Call ID is generated by backend, we'll get it from CALL_RINGING or CALL_STATUS
      // For now, we'll track the call type and wait for backend response
  ...
  async answerCall(callId: string, conversationId: string): Promise<void> {
    ...
    // Get user media
    try {
      // Get call type from incoming call data (stored when CALL_INCOMING received)
